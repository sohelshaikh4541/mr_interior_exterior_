...
import 'package:interior_v_1/model/get_deduction_package.dart';
...
class _AddProjectState extends State<AddProject> {
  ...
  late TextEditingController _finalPriceController = TextEditingController();
  ...
  DropdownItemDeduct? _selectedCategoryType;
  ...
  void _updateTotalAmount() {
    ...
    selectedProduct.forEach((key, value) {
      int unitPrice = value[0];
      total += unitPrice;
    });
    setState(() {
      totalAmount = total;
    });
    double finalTotal = double.parse(_priceController.text.replaceAll(',', ''));
    double result = finalTotal - totalAmount;

    final formattedResult = NumberFormat('#,##,##0.00', 'en_US').format(result);

    _finalPriceController = TextEditingController(text: formattedResult);
  }
  ...
  Future<void> _postPrice() async {
    ...
    setState(() {
      _response = response;
      isLoading = false;
      _priceController = TextEditingController(text: '${response['data']['price']}.00');
    });
  }

  Future<List<DropdownItemDeduct>> _fetchDeductionPackage(String filter) async {
    final deductItem = await StatesServices().getDeductionPackage();
    if (deductItem.status != true) {
      return [DropdownItemDeduct(id: -1, items: 'No data found',deduction_rate: 'No data found')];
    }

    final deduction = deductItem.data ?? [];
    if (filter.isEmpty) {
      return deduction
          .map((deduction) => DropdownItemDeduct(
          id: deduction.id!, items: deduction.items ?? 'Unknown Source', deduction_rate: deduction.deductionRate ?? 'Unknown Source'))
          .toList();
    } else {
      return deduction
          .where((deduction) =>
      deduction.items != null &&
          deduction.items!.toLowerCase().contains(filter.toLowerCase()))
          .map((deduction) =>
          DropdownItemDeduct(id: deduction.id!, items: deduction.items!, deduction_rate: deduction.deductionRate!))
          .toList();
    }
  }
              ...
              child: Column(
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: DropdownSearch<DropdownItemDeduct>(
                      dropdownDecoratorProps: DropDownDecoratorProps(
                        dropdownSearchDecoration: InputDecoration(
                          hintText: "Select Deduction Item",
                          border: OutlineInputBorder(
                            borderRadius: BorderRadius.circular(8.0),
                            borderSide:
                      ...
                      onChanged: (DropdownItemDeduct? newValue) {
                        if (newValue != null) {
                          setState(() {
                            ...
                            isItemSelected = true;
                            _selectedItemProduct = newValue.items;
                            selectedProduct[newValue.items] = [
                              int.parse(newValue.deduction_rate),
                            ];
                            _updateTotalAmount();
                          });
                        }
                      ...
                      popupProps: PopupProps.menu(
                        ...
                        searchFieldProps: TextFieldProps(
                          decoration: InputDecoration(
                            ...
                            contentPadding: EdgeInsets.symmetric(
                                horizontal: 10, vertical: 10),
                            hintText: "Search Deduction Rate",
                          ),
                        ),
                        itemBuilder: (context, item, isSelected) {
                          return ListTile(
                            title:
                            Text(item.items, style: TextStyle(fontSize: 16)),
                            trailing: Checkbox(
                      ...
                      asyncItems: (String filter) async {
                        return await _fetchDeductionPackage(filter);
                      },
                    ),
                  ),
                  ...
                  CustomTextField(
                    ...
                    hintText: 'Flat No./Plot No.',
                    icon: CupertinoIcons.number_circle,
                    validator: (value) {
                      if (value == null || value.isEmpty) {
                        return 'Please Enter Flat No/Plot No';
                      }
                      return null;
                    },
                    showError: _showErrors,
                  ...
                  if (_locStatus && _currentPosition == null)
                    Padding(
